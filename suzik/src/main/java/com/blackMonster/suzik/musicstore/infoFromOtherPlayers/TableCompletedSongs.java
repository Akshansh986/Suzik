...
public class TableCompletedSongs {
	...
	public static void createTable(SQLiteDatabase db) {
		String sql = String
				.format("create table %s"
						...
						TABLE, C_ID, C_TRACK, C_ARTIST, C_ALBUM, C_DURATION,
						C_STREAMING, C_COMPLETED_TS);
		db.execSQL(sql);
	}
	...
	public static BroadcastSong search(Song song, Context context) {
		...
		Pair<String, String[]> args = DbUtils.songToWhereArgs(song, C_TRACK,
				C_ARTIST, C_ALBUM, C_DURATION);

		Cursor cursor = db.query(TABLE, null, args.first, args.second, null,
				null, null);
	...
	public static boolean remove(BroadcastSong song, Context context) {
		SQLiteDatabase db = DbHelper.getInstance(context).getWritableDatabase();
		Pair<String, String[]> args = DbUtils.songToWhereArgs(song, C_TRACK, C_ARTIST, C_ALBUM, C_DURATION);
	...
	public static Cursor getAllRows(Context context) {
		SQLiteDatabase db = DbHelper.getInstance(context).getReadableDatabase();
		return db.rawQuery("select rowid _id,* from " + TABLE + " ORDER BY "
				+ "_id" + " DESC", null);
	}
